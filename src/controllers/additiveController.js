// additiveController.js

const axios = require('axios');

const MENSA_API_BASE_URL = 'https://mensa.gregorflachs.de/api/v1';
async function fetchAdditives() {
    try {
        const endpoint = `${MENSA_API_BASE_URL}/additive`;
        console.log('Fetching additives'); // Neues Log hinzugefügt
        const response = await axios.get(endpoint, {
            headers: {
                'X-API-KEY': '',
            },
        });
        console.log('Additives fetched successfully'); // Neues Log hinzugefügt
        return response.data;
    } catch (error) {
        console.error('Error fetching additives:', error);
        throw error;
    }
}

module.exports = { fetchAdditives };
